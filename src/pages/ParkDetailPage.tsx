  ...
  if (error || !park) {
    return (
      <div className="text-center py-12">
        <h2 className="text-2xl font-bold text-text mb-4">Error Loading Park</h2>
        <p className="text-text/70 mb-6">{error?.message || 'Park not found'}</p>
        <button 
    ...
    <div>
      <div className="relative h-64 md:h-96 mb-8 rounded-xl overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 p-6 z-20">
          <h1 className="text-3xl md:text-4xl font-bold text-text mb-2">{park.name}</h1>
          <p className="text-lg text-text/90">{park.location}</p>
        </div>
      </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {park.activities.map((activity, index) => (
                    <span key={index} className="bg-primary/20 text-primary-light px-3 py-1 rounded-full">
                      {activity}
                    </span>
                  ))}
                </div>
          ...
          <div className="card">
            ...
            <ul className="space-y-3">
              <li className="flex items-start">
                <svg className="h-6 w-6 text-primary-light mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
              ...
              <li className="flex items-start">
                ...
                <span>Educational visitor center with interactive exhibits</span>
              </li>
            </ul>
          </div>
        </div>
        ...
        <div>
          <div className="card sticky top-4">
            ...
            {!showTripForm ? (
              <div>
                <p className="text-text/80 mb-6">
                  Ready to explore {park.name}? Create a trip plan to organize your visit.
                </p>
                <button
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => date && setTripData({...tripData, startDate: date})}
                        className="input w-full"
                    ...
                    <div>
                      ...
                      <DatePicker
                        ...
                        onChange={(date) => date && setTripData({...tripData, endDate: date})}
                        className="input w-full"
                        dateFormat="MMM d, yyyy"
